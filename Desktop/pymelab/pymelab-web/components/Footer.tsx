...
export default function Footer() {
  ...
  return (
    <footer className="border-t border-[#2A2A2A] bg-[#0A0A0A]">
      ...
      <div className="max-w-7xl mx-auto px-6 py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-12">
        ...
        <div className="lg:col-span-1">
          <Link href="/" className="flex flex-col leading-none mb-4">
            <span className="font-display text-3xl font-light italic text-[#F0EDE8]">
          ...
          <div className="flex gap-3">
            {[
              ...
              <a
                key={label}
                href={href}
                className="w-9 h-9 border border-[#2A2A2A] flex items-center justify-center text-[#666] hover:border-[#C8A96E] hover:text-[#C8A96E] transition-all duration-200 font-light text-xs tracking-wider"
              >
                {label}
              </a>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-2.5">
            {footerLinks.services.map(({ label, href }) => (
              <li key={label}>
                <Link href={href} className="text-sm text-[#888] hover:text-[#F0EDE8] transition-colors duration-200">
                  {label}
                </Link>
              </li>
            ))}
        ...
        <div>
          ...
          <ul className="space-y-3">
            <li>
              <a href="mailto:contacto@pymelabagency.com" className="flex items-center gap-2.5 text-sm text-[#888] hover:text-[#F0EDE8] transition-colors duration-200">
                <Mail size={14} className="text-[#C8A96E] shrink-0" />
                contacto@pymelabagency.com
              </a>
            </li>
            <li>
              <a href="tel:+34618805348" className="flex items-center gap-2.5 text-sm text-[#888] hover:text-[#F0EDE8] transition-colors duration-200">
      ...
      {/* Bottom */}
      <div className="border-t border-[#1A1A1A] px-6 py-5 max-w-7xl mx-auto flex flex-col sm:flex-row items-center justify-between gap-3">
        ...
        <div className="flex gap-5">
          {[
            ...
            <Link key={key} href={href} className="text-xs text-[#444] hover:text-[#888] transition-colors duration-200">
              {t(key)}
            </Link>
          ))}
        </div>
